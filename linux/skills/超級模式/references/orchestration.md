...
### 派工前置 — 確認 Codex 最新版
跑 `scripts/codex-check.sh`：比對 `codex --version` 與 `npm view @openai/codex version`、印出 UP-TO-DATE / BEHIND / AHEAD / CURRENT / UNKNOWN 判定（2026-09-24 訂正 9/6 D8：原寫只有 UP-TO-DATE / OUTDATED 兩態），並做 read-only smoke test。Linux 版**沒有**能力面 baseline diff（`--update-baseline` 只在 Windows／macOS 版）。**24 小時內查過會直接回快取結果**（存 `~/.claude/.codex-check-last`；smoke 失敗會刪快取，壞掉的 codex 不會被舊快取報成 OK；快取只看時間、不因版本變更作廢，升級後要 `-f`），`-f` 強制重查。**BEHIND＝中性情報、不是更新指令**，要不要更新由使用者決定（**先問使用者**，更新 global 工具屬系統變更）：`npm install -g @openai/codex@latest`。壞了就 `npm install -g @openai/codex@<舊版>` 釘回去。
...
> 一般模式（超級模式 OFF）不受本節限制，plugin 全部可用。但全域「Codex 討論夥伴」規則不變：決策型輸出仍走 `codex-consult.sh -n`（`--no-credential`）（2026-09-23 事實訂正：原誤用 Windows 討論旗標，POSIX 腳本會報 unknown arg、exit 2）——它吃 brief（審**還沒動手**的決策），plugin 吃 git state（審**已寫出**的碼），互相取代不了。
