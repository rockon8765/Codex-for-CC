...
- **做法**：簡報用 Write 工具寫進 scratchpad → 用 Bash 工具跑
  `~/.claude/skills/超級模式/scripts/codex-consult.sh -d <相關 repo；無 repo 用目前工作目錄> -n -f <brief>`（工具 timeout 360000ms；`-n` = 討論模式，不 mint 超級模式憑證）。
- **簡報必須逼 Codex 當反方**：附上 Claude 初判，明文要求它：反對初判、給失敗情境、給替代排序、標注信心。不准寫成引導它附和的簡報。
