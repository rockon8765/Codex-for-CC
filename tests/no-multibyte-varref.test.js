 ...
 *
 * ⚠️ **寫作慣例**（本規則在開發期間抓到我三次，全部是在**中文註解**裡）：
 *    用中文寫關於某個變數的說明時，後面自然會接全形標點（「，」「（」「）」），
 *    於是 `$VAR，` 就命中規則。這不是誤報 —— shell 不區分「這行是註解」，
 *    註解裡的寫法會被複製到程式碼裡。**在 CJK 文字旁邊提到變數時一律寫 ${VAR}**。
 *    （規則刻意保持笨而嚴：會解析註解的聰明規則比較容易被騙。）
 *
