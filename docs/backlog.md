...
| ~~**`codex-check` 把「空區段」誤報成 UNPARSEABLE**~~ **已修（2026-08-08）** —— 三態改寫、單一 parser 同時產出 items 與 evidence；驗證見 `HANDOFF-macos-codex-check-2026-08-08.md`。留紀錄避免重複開題。原始描述：| 2026-08-08 實測：移除 `superpowers` 外掛後 `~/.codex/config.toml` 的 `[hooks.state]` 成為**合法的空表**（下一個非空行就是 `[shell_environment_policy.set]`），但 `codex-check` 回報「有 hooks.state 段但解析 0 筆，疑序列化格式變更」，並連帶把整個能力面列進「UNKNOWN 段（無法與 baseline 比對）」。**空 ≠ 解析失敗**：現況既擋住比對，又是 cry-wolf——真的格式變更時這個警告已無訊號價值。修法：區分「區段不存在」「區段存在但零筆」「真的解析失敗」三態 |
| ~~**三支腳本硬編 `C:\npm\codex.cmd`**~~ **部分處理（2026-08-08）** —— `codex-check` 現在會在 cache gate 前印出「使用的 codex: <path>」，並在 PATH 上是另一支時警告（**只比對路徑、不執行**）。⚠️ **解析方式刻意未改**：`C:\npm` 是為了繞 MAX_PATH 才選的，改成 PATH 解析屬三平台行為變更，仍未做；`codex-consult`／`codex-exec` 也仍無覆寫機制。原始描述：| 三支都寫死該路徑（`codex-check.ps1:21` 可用 `$env:CODEX_CHECK_CODEX_CMD` 覆寫，**`codex-consult.ps1:35` 與 `codex-exec.ps1:33` 無任何覆寫機制**），所以 skill 永遠不看 PATH。**2026-08-08 的活案例**：維護者把原生安裝升到 0.147.0，但 npm 全域仍是 0.146.0，於是互動 `codex` 顯示 0.147、skill 實際驅動的 worker 卻是 0.146，兩邊都「正確」但不一致，且沒有任何地方提示這個落差。⚠️ **不要順手改成 PATH 解析**：`C:\npm` 這個位置當初是為了繞 MAX_PATH 才選的，屬三平台行為變更。最小改善是在能力面盤點時印出實際使用的 codex 路徑，並在它與 PATH 上的 `codex` 版本不同時提示 |
