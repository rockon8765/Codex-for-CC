...
| **`MIGRATION` §1 的 probe 對「JSON 合法但 schema 壞掉」不 fail-closed（2026-08-04 實測）** | `docs/MIGRATION-hook-settings-target.md` 的 `count()` 直接寫 `for (const entry of (j.hooks && j.hooks.PreToolUse) \|\| [])`，沒有逐層驗形狀。對 `main` 現況實測三種輸入：`settings.json` 內容為 `null` → `TypeError: Cannot read properties of null (reading 'hooks')`；`PreToolUse` 是物件 → `TypeError: object is not iterable`；**`PreToolUse` 是字串 → `exit=0`，逐字元迭代後靜默數成 0，印出「兩邊都沒有 gate…照 `AI-INSTALL` 步驟 2 重做」**。第三種最糟：假陰性之後還把使用者導去重跑安裝，而重裝正是可能造成重複註冊的動作。**重現**：抽出 §1 的 probe 存成 `.js`，用假 `HOME` 塞上述三種 `settings.json` 各跑一次。修法方向：逐層驗 object／array，異形一律判 schema-invalid、停止並回非零，並補矩陣 fixtures |
| **`MIGRATION` §2.2 的「整個條目搬移」可能製造重複註冊或夾帶其他 hook（2026-08-04 檢視）** | §2.2 要求把 `settings.local.json` 裡含 `super-mode-consult-gate` 的 **`hooks.PreToolUse` 整個條目**搬進 `settings.json`。兩條疑慮：(a) `settings.json` **已有** gate 時，搬移＝**重複註冊**——而 §1 的判定表把 `main≥1 && local≥1` 也導向 §2；(b) 該 outer entry 若還掛著**其他** hook，會把不相關的 hook 一併升到 user scope。**這與規劃書 §2（A2「修復文件會製造重複註冊」）是同一條失敗鏈**，A2 尚未做。⚠️ **兩項均為對文件的靜態檢視 ＋ probe 的實跑，未在真實受影響環境端到端驗證**；且它們出自 2026-08-04 一個敘述可靠度偏低的 session（見該日 `~/.claude/super-mode-logs/codex_consult_20260804_*` 六份逐字稿），**動手前請自行復現一次**，不要直接採信本列的因果敘述 |
