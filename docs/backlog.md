...
| **⚠️ 未定案：`~/.claude/settings.local.json` 會不會被載入（影響 macOS／Linux 的 hook 是否真的生效）** | macOS／Linux 安裝指引把 hook 註冊進 `~/.claude/settings.local.json`。[官方 settings 文件](https://code.claude.com/docs/en/settings)說 user scope **只有** `~/.claude/settings.json`、`settings.local.json` 是**專案**層級——若屬實，照指引安裝的 macOS／Linux 使用者 **hook 從未被註冊**，而且 `matcher-contract.test.js` 的候選清單含該檔、找到就 PASS，**那支防假綠的測試自己就是假綠來源**。但 2026-07-28 字串搜尋本機 Claude Code 執行檔，發現它確實有主動處理「legacy settings.local.json」（read／transform／revoke 三種訊息），無法從字串分辨 scope。**兩邊證據矛盾，未定案**。驗證程序見 [`verify-settings-scope.md`](verify-settings-scope.md)（含必要的對照組）。**測出結果前不要改 snippet 或指引** |
