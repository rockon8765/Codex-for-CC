...
| **`Get-TreeFingerprint` 的比對界線** | 1b 驗證 skill 備份是否與 live 一致時只列**檔案**（`Get-ChildItem -File`），因此不驗空目錄、也不驗 reparse metadata。若某個 provider／PowerShell 版本把來源與備份都靜默列成空集合，兩邊 fingerprint 會相等而通過。目前 payload 沒有空目錄，且 `$ErrorActionPreference='Stop'` 通常會攔下列舉錯誤，所以**未修**。要收緊就得把目錄與 link 型態一併納入指紋 |
