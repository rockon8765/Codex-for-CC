...
| ~~**macOS 原生驗證（2026-07-27 批次）**~~ **已完成（2026-07-31）** | 該批已在 Mac 真機驗完，且涵蓋分支尖端 `6f7839b`——詳見下方「2026-07-27 批次的 macOS 原生驗證」列。promote 進 main 後即可刪驗證分支 `refactor/context-engineering-2026-07-27-pending-native-macos` |
...
| ~~2026-07-27 批次的 macOS 原生驗證~~ **已完成（2026-07-28；尖端 2026-07-31 複驗）** | SHA `53cbc5f` 於 macOS 26.5.2 arm64 / Claude Code 2.1.163 驗證：7 筆 blob 相符、`gate-cases` **117/117**、`matcher-contract` exit 0、`consult-schema` **4/4**、`run-e2e` **11/11**（`GATE_UNDER_TEST` 確認指向 worktree）。同批的 `tests/ai-install/run-posix.sh` 在 BSD userland（bash 3.2.57）**59/59**、反向驗證 7 FAIL 與 Linux 一致——確認 GNU→可攜移植與 symlink 守衛的 fail-open 修正在 macOS 成立。交接文件：[`HANDOFF-macos-2026-07-28.md`](HANDOFF-macos-2026-07-28.md)。**2026-07-31 補驗分支尖端 `6f7839b`**——`53cbc5f` 之後又有 7 個 commit，原本核對的 7 筆 blob **有 6 筆變動**（gate hook／snippet／SKILL／`matcher-contract`／`AI-INSTALL`／`run-posix.sh`），故整批重跑：`gate-cases` **117/117**、`matcher-contract`（新版排他驗證）exit 0、`consult-schema` **4/4**、`run-e2e` **11/11**（`GATE_BLOB=f1781d6e59a06c78d43ae074545f08ea5f0740d3`）、`run-posix.sh` **64/64**；反向驗證對 `53cbc5f` 的 `AI-INSTALL` **5 FAIL**（settings-target 新斷言）、對 `06adac5` **12 FAIL**（5 + 原本 7 個 symlink）。**分支尖端在 macOS 為綠，可 promote** |
| **`codex-check` 把「空區段」誤報成 UNPARSEABLE** | 2026-08-08 實測：移除 `superpowers` 外掛後 `~/.codex/config.toml` 的 `[hooks.state]` 成為**合法的空表**（下一個非空行就是 `[shell_environment_policy.set]`），但 `codex-check` 回報「有 hooks.state 段但解析 0 筆，疑序列化格式變更」，並連帶把整個能力面列進「UNKNOWN 段（無法與 baseline 比對）」。**空 ≠ 解析失敗**：現況既擋住比對，又是 cry-wolf——真的格式變更時這個警告已無訊號價值。修法：區分「區段不存在」「區段存在但零筆」「真的解析失敗」三態 |
| **`codex-check`／`codex-consult`／`codex-exec` 硬編 `C:\npm\codex.cmd`** | 三支都寫死該路徑（`codex-check.ps1:21` 可用 `$env:CODEX_CHECK_CODEX_CMD` 覆寫，**`codex-consult.ps1:35` 與 `codex-exec.ps1:33` 無任何覆寫機制**），所以 skill 永遠不看 PATH。**2026-08-08 的活案例**：維護者把原生安裝升到 0.147.0，但 npm 全域仍是 0.146.0，於是互動 `codex` 顯示 0.147、skill 實際驅動的 worker 卻是 0.146，兩邊都「正確」但不一致，且沒有任何地方提示這個落差。⚠️ **不要順手改成 PATH 解析**：`C:\npm` 這個位置當初是為了繞 MAX_PATH 才選的，屬三平台行為變更。最小改善是在能力面盤點時印出實際使用的 codex 路徑，並在它與 PATH 上的 `codex` 版本不同時提示 |
| **settings target 批次的後續工作（A1／A2／B1／C／D）** | **執行載體＝[`settings-target-followup-2026-08.md`](settings-target-followup-2026-08.md)**（2026-08-08 重新盤點）。它取代了 2026-07-28 的規劃書 v4——v4 的事實大致仍成立，但作為執行合約已失效（§7 自相矛盾、§9 把已完成項列為未完成、基準數字過時、A1 呼叫點表有實質錯誤：把 `linux-platform-notes.md` 的**驗證紀錄表**列為要修改的呼叫點）。v4 原文封存在 annotated tag `archive/remediation-plan-2026-07-28`。**下一列的 MIGRATION 缺陷是本批的一部分**，因為已先行 containment 才單獨列出 |
| ~~**`MIGRATION-hook-settings-target.md` 第 2 節不可執行**~~ **已修（2026-08-08，同日）** | 留紀錄避免重複開題；驗證見 [`settings-target-followup-2026-08.md`](settings-target-followup-2026-08.md) §4.2（probe 九種輸入 9/9，含對照組）。原始三個缺陷，來源是對 `main` 的靜態檢視加上把該文件第 1 節的 probe 原樣抽出、以假 `HOME` 餵四種 `settings.json`（`null`／物件／字串／正常陣列，最後一種為對照組）在 Node v24.16.0 實跑。**(a) probe 對「JSON 合法但形狀不對」不 fail-closed**：`hooks.PreToolUse` 是**字串**時 `exit 0` 且把 gate 數成 0，判定成「兩邊都沒有 gate……照 `AI-INSTALL` 步驟 2 重做」——假陰性且把使用者導去重裝，而重裝正是可能造成重複註冊的動作；`null`／物件則以 `TypeError` 中斷（非零，但只印 stack trace）。**(b) 判定表的 `≥1／≥1` 在第 2 節沒有對應分支**：判定表寫「要**移除** local 那份」，但 2.2 是無條件的「整個條目**搬**進 `settings.json` 再從 local 刪掉」，`settings.json` 已有一筆時照做會變兩筆。**(c) 2.2 的「整個條目」粒度過粗**：該 outer entry 若還掛著其他 handler，整筆搬移／刪除會連帶動到不相關的 hook。**修法方向**：probe 逐層驗 object／array，異形一律判 schema-invalid 並回非零；第 2 節改成 handler 粒度並補「main 已有一筆」的分支；寫死後置條件 `settings.json` gate handler ＝1、`settings.local.json` ＝0；補四種形狀的 fixtures。⚠️ **未**在真實受影響的 macOS／Linux 環境端到端驗證 |
