...
> （見 [`AI-INSTALL.md`](AI-INSTALL.md) 步驟 2、[`MIGRATION-hook-settings-target.md`](MIGRATION-hook-settings-target.md) 第 4 節）。
> 現行說法是：該檔有多個寫入者，**Claude Code 自己的 plugin manager 就是其一**
> （top-level key 除 `hooks` 外還有 `enabledPlugins`），但**沒有證據顯示任何工具會覆寫或移除 `hooks` 段**。
>
