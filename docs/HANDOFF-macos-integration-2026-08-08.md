...
> ## ⛔ 這份驗證已作廢，需要重跑（Codex 複審後又改了受測檔）
>
> 對 `319299c` 的驗證確實全綠，**但 Codex 第二次審查（判定 BLOCK）之後又改了
> macOS 受測檔**（`codex-check.sh` 的形跡規則、三平台 `matcher-contract.test.js`
> 的 type 判定、三份 snippet）。**blob 已不同，那次結果對現在的 tip 不成立。**
>
> 待整批定稿後會重釘 SHA／blob 再發一次。**在那之前不要拿下方數字當通過依據。**
>
> 下方保留該次驗證的紀錄（對 `319299c`，已非現行 tip）：
>
> ## （已作廢）2026-08-08 對 `319299c` 的驗證
>
