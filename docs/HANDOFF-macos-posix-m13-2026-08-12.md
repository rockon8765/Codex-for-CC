...
> `id -u`＝501／git 2.55.0／HEAD `392e9e2`／`git status --porcelain` 為空／
> 兩個 `.sh` 的 CR 計數皆 0／四筆 blob 全符／未使用 node。
>
> | # | 實測 | |
> |---|---|---|
> | B-1 | `PASS=125 FAIL=0` rc=0，文件 hash `a2b3d69f…` | ✅ |
> | B-2 | `PASS=107 FAIL=18` rc=1，文件 hash `46c3cb01…`，18 條與 §3 **逐條相符** | ✅ |
> | B-3 | `TOTAL 13 PASS 13 FAIL 0` rc=0 | ✅ |
> | B-4 | `PASS=124 FAIL=0` ＋ `STOP 案數不符：實跑 124、預期 125（uid=501）` rc=1 | ✅ |
> | B-5 | `PASS=85 FAIL=1`（合計 86）、無 STOP、rc=1 | ✅ |
> | B-6 | `PASS=125 FAIL=0` rc=0 | ✅ |
> | B-7 | `PASS=125 FAIL=0` rc=0 | ✅ |
> | **B-8** | **`PASS=123 FAIL=2` rc=1**，紅的恰好是兩條 `中止後整個假 HOME 未變` | ✅ |
>
> **`[M13][bak]／[live] 列舉失敗 → 回滾中止` 仍不在 B-2 的 FAIL 清單**（與 Linux 一致）。
>
> ⭐ **驗證者做的三件「不只信綠燈」的事**（比報數字有價值得多）：
> 1. **正向對照**：確認 bash 3.2.57 真的會為非互動腳本 source `$BASH_ENV`（側檔實際被寫出 1 byte）、
>    B-6 的 glob 真的命中 harness 佈局、B-7 的 payload 下裸 `bash -n` 回 77 而 `command bash -n` 回 0。
> 2. **A／B 拆防護**（暫存副本、repo 未動、各只差一行）：拆掉 `run()` 的 `BASH_ENV= ENV=` → `123/2`
>    紅在兩條 `中止後整個假 HOME 未變`；拆掉 `bash -n` 那行的 `command` → `123/2`
>    紅在兩條 `產出的區塊語法正確（bash -n）`。**與 §0 第 7／13 點記載的數字與斷言精確吻合。**
> 3. **B-8 的反向對照**：把**舊 blob `90ddbd10…`** 從 object store 取出跑同一個 mutant →
>    **`PASS=125 FAIL=0` rc=0（mutant 完全存活）**。
>    **這是獨立重現本文件 §0 第 15 點的宣稱，不是採信它。**
>    並確認 `unlock_tree` 函式與三個呼叫點在新版確實已刪除（剩下 2 處 grep 命中是註解）。
>
> 驗證者另外抓到本文件一處錯誤（§4 的項目範圍漂移，**且是 §5 訂正 #4 的原樣復發**）
> 與一處可收緊之處，**均已修正**，見 §5。
>
> ### 上一輪（blob `90ddbd10…`，已作廢）的結果，保留存證
>
...
| B-8 | **mode 型 mutant 必須被抓到**（見下）| `PASS=123 FAIL=2`、exit 1，紅的**恰好**是這兩條（**名稱寫全，不縮寫**）：`[M13][bak] 中止後整個假 HOME 未變（hook／settings 也在內）`、`[M13][live] 中止後整個假 HOME 未變（hook／settings 也在內）`。同案的窄 oracle（`中止後 live 未變`）與注入自我檢查應維持**綠** —— mutant 只改 mode 不改內容，只有記 mode 的寬 oracle 看得到 |
...
- **§2 表格裡的每一項**的實際輸出（`PASS=`／`FAIL=`／`rc=`）。
  ⚠️ **這裡刻意不再列出項目編號範圍。** 原本寫「B-0…B-6」、修成「B-0…B-7」、加了 B-8 之後
  又忘了同步 —— **同一個漂移發生兩次**（見 §5 訂正 #4 與 #5）。
  §2 的表是唯一的項目清單，這裡只指向它，不複述。
- 有 FAIL 的項目請貼**完整 FAIL 清單**逐條核對（目前是 B-2 與 B-8）
- 特別確認：
  1. `[M13][bak]／[live] 列舉失敗 → 回滾中止` **不**出現在 B-2 的 FAIL 清單（平台一致性）
  2. B-6／B-7 在 bash 3.2 下仍是 125/0，**而且注入真的有觸發**（正向對照，見 §2 各項的說明）
  3. **B-8 真的抓到，而且紅的就是那兩條**（同案的窄 oracle 應為綠 —— mutant 只改 mode 不改內容）
...
### 2026-08-13 第二次 macOS 驗證又抓到的（已修）

| # | 問題 | 處置 |
|---|---|---|
| 5 | **§4 的項目範圍漂移 —— 這是上面 #4 的原樣復發。** #4 修的是「§4 寫 B-0…B-6，§2 卻定義到 B-7」；這輪把 B-8 加進 §2，又忘了同步 §4（仍寫 B-0…B-7）。連帶「特別確認的預測」也沒把本輪最重要的 B-8 列進去。 | **不再改數字，而是移除漂移來源**：§4 改成指向「§2 表格裡的每一項」，**不複述編號範圍**。同一份文件裡把同一份清單寫兩遍，就是這個缺陷的成因 |
| 6 | §2 的 B-8 期望把第一條斷言名稱**縮寫**（省略共用後綴）。判定雖無歧義，但本文件的判準明文是「釘斷言名稱」，而 #2 修的正是「斷言名稱截斷」——同一個坑 | 兩條名稱都寫全，並補上「同案的窄 oracle 與注入自我檢查應維持綠」這條反面判準 |

⚠️ **這兩項的共同點**：#4／#5 是同一個漂移、#2／#6 是同一個截斷。
**在一份以「釘名稱、釘 blob」為判準的文件裡，判準本身被複述兩份就一定會漂。**
