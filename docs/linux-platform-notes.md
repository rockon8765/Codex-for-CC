...
| `scripts/codex-exec.sh` | ❌ **不相同**（2 行）：macOS 有 `--disable remote_plugin`，Linux **沒有** | ⚠️ **這不是 Linux 落後，是三平台不一致**——`windows/codex-exec.ps1` 同樣沒有；該旗標是 Mac 端單方面加的硬化（`8bbd43f`）。維護者已明確**暫緩**收緊 `--disable`，故本版**刻意不移植**。要改請三平台一起改。⚠️ 2026-09-14 訂正：該旗標只關遠端外掛目錄（上游 #28443），不是硬化；註冊面有效的是 `--disable apps --disable plugins`（見 [`ACCEPTANCE-capability-boundary-2026-09-14.md`](ACCEPTANCE-capability-boundary-2026-09-14.md)） |
