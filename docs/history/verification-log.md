...
> （`docs/X` → `../X`、其餘 → `../../X`）。**正文只有一處刻意編輯**：末尾「仍未涵蓋的」那段原寫「見安裝節」，
> 拆檔後已失去上下文，改成明確指向 `README.md` 的「安裝」節（已用 diff 確認正文只有這 1 行與來源不同）。
> **其餘一字未改**——包括當時寫下的
...
|---|---|---|---|---|
| 2026-08-09 發現 → 08-10 修畢 | 驗證資產假綠修正 ＋ macOS 原生驗收 | `a85e88a`／`4414ae7` | Win／Linux／mac | ⚠️ 段內的「最終驗收狀態（**tip**）」用了未釘 SHA 的 `<tip>`，且該段「共 7 個檔案」只對**當時**成立——區間 `a85e88a..ce01737`（`ce01737` ＝ 2026-08-18 觀測到的 `origin/main`）實測已有 36 commits／26 檔案。**此處刻意釘死兩端 SHA、不寫 `HEAD`**，否則這個數字每 commit 就漂一次 |
