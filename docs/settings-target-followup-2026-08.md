...
## 4.5 Codex 合併前審查（2026-08-08，判定 BLOCK → 已修）

首次送審回 **BLOCK**，六條 finding 逐條對磁碟查證後，**四條屬實且都是本批引進的**。

| # | 內容 | 處置 |
|---|---|---|
| **F1** | `AI-INSTALL` 步驟 2 自抄的簡化矩陣把「`settings.json`=0、`local`≥1」導向 **B**（純減法），但 `MIGRATION` 的矩陣是 **A**。照唯一真相操作會**刪掉受影響使用者僅存的 gate**，且發生在動過未必完整備份的 local 之後。三份 snippet 複製同一錯誤 | **採納**。修法**不是**補成完整矩陣＋一致性測試，而是**移除重複**——兩份各留一份矩陣遲早再度分歧，測試只保證當下一致。`AI-INSTALL` 與三份 snippet 改成「跑 probe，照它印的判定走」 |
| **F2** | probe 與三平台 matcher 判「已註冊」只比對 `command` substring，不驗 `type` | **採納**。命中 needle 即強制 `type === "command"`；並把「type 不對」與「完全沒註冊」分開報——併在一起會讓使用者去「重跑步驟 2」而變成重複註冊 |
| **F4／F4b** | 第四條洗白路徑（`[hooks] # 註解`）；macOS 的 items 走 `sed`（不吃縮排）、evidence 走 `awk`（吃縮排並當已認得跳過）→ 縮排 canonical 表頭「抽不到又不報」；混合案（1 認得＋1 異形）因 items>0 跳過形跡檢查 | **採納**。改成**單一 pass 同時產出 items 與 evidence**；表頭先抓 `[…]` 內容再比對；**形跡不再受 items 數影響**。動手前列 14 種 TOML 寫法一次驗完 |
| **F5** | 路徑輸出在 24h cache gate **之後**，最常見的重跑路徑看不到 | **採納**，移到 gate 前並加 cache-hit 斷言（反向驗證恰 1 FAIL）|
| **F6** | 兩份 handoff 標「已完成」，但其後 commit 又改了受測檔 | **採納**，改回 pending 並明寫「重釘前不要拿舊數字當通過依據」|
| **F3** | B1 未證明備份可讀就先刪 live，要 staging＋hash manifest＋quarantine | **部分駁回**。那正是 [`installer-rewrite-spec.md`](installer-rewrite-spec.md) 的設計；B1 是窄範圍 hotfix，「先刪後還原」是**既有行為不是本批回歸**。把 B1 擴成 rewrite 等於在沒有測試臺的 markdown 裡再蓋一層狀態機。觀察成立，已記在該 spec |

> **本輪最值得記的教訓**：F1 與 F4b 都是**同一份規則寫兩遍**造成的——一份在 `AI-INSTALL`
