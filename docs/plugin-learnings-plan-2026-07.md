...
- **T7b 實作完成殘留風險清單（2026-07-08；Codex commit-gate PROCEED + Workflow 雙代理對抗審 over/under-permit）**：實作＝SP 4 精確字串 + Notion UUID 形狀 regex（收緊為標準 8-4-4-4-12、大小寫 hex）+ fail-closed helper `isMcpPathlessAllowed`（string===／RegExp.test／else return false）+ harddeny 訊息含 sanitized 被擋 tool 名。雙代理審**無 CRITICAL/HIGH、全 fail-closed**。**已知/已接受風險**：(1) Notion 是「UUID 形狀信任」非特定 connector id——任何 8-4-4-4-12 形狀 server 暴露 `notion-(create-pages|update-page|create-comment)` 即放行（攻擊需仿冒 server＋一張有效憑證，門檻非零）；(2) Notion schema 寫入（`update-data-source`/`create-database`）**刻意 under-permit**，超級模式 session 內動 DB schema 會 harddeny，需要時另開裁決擴白名單；(3) SP 前綴 `sportspredict` 為精確字串、大小寫敏感，搬機/重裝/改 server key 會 under-permit（fail-closed 安全），改 key 須同步三平台 hook；(4) Notion regex 假設官方 hosted connector 恆為標準 hex UUID 且工具名不變，非 UUID 掛載或官方改名即 under-permit；(5) pathless-allow 免 repo 綁定但仍需 20 分鐘憑證，跨 repo 殘留憑證於窗內可觸發（既定設計）；(6) MCP 唯讀子字串啟發式免憑證＝既存正交，T7b 未使其變寬；(7) 安裝版 `~/.claude/hooks/` 未含 T7b，須待 **T7-S4** 同步才在生產生效——別把「測試綠燈」誤讀為「已上線保護」。
...
### 0.1 T5-S0 GATE 結果（2026-07-08，批次 5）：**FAIL → T5 結案未實作**

`codex exec resume`（codex-cli 0.142.5 / Windows）**不可被視為繼承 origin session 的 sandbox**。實測（Claude 親跑、超級模式外、隔離 temp repo；probe 設計先經一個唯讀 Claude 子代理對抗審，把判準從「沒寫檔」升級為「強制寫入嘗試＋擷取 OS 拒絕訊號」——否則模型「只是不試」會 false-PASS）：

- fresh `codex exec --sandbox read-only` 寫入嘗試 → OS 拒絕（`Set-Content` exit_code 1、「拒絕存取路徑」），檔案未建 → read-only 為真 OS 級 enforcement。
- read-only-origin session 經 `codex exec resume <id>`（resume 子指令**無 `--sandbox` 旗標可傳**）→ challenge-response 確認 session 連續性成立（本輪 fresh token + 上輪記憶 nonce 正確拼接），但 resumed turn `[IO.File]::WriteAllText(RESUME_WRITE_PROBE.txt,'ok')` **exit_code 0、無拒絕、檔案建立成功**。
- **結論：session continuity ≠ sandbox continuity。** T5 賴以成立的「resume 沿用 read-only」前提不成立 → 依 T5-S0 判準＝**FAIL → 中止 T5、結案未實作 S1–S4**。

**現行不受影響**：`codex-consult.ps1` 用 `--sandbox read-only --ephemeral`（session 不落地、不可 resume），此漏洞碰不到；無 retroactive 曝險。

**重開條件**：未來若要重試 T5，必須先新增並通過 **per-resume sandbox override GATE**（例如 `codex exec resume -c sandbox_mode=read-only`），且該 override 須：resume 確認接受、unsupported config **fail-closed**、resumed turn 外部寫入被擋、三平台語法一致、腳本每次 resume 強制帶、文件改為「origin read-only 不可信、per-resume enforcement 才可信」。未通過前維持 `--ephemeral` 單輪諮詢。

證據 log：`%TEMP%\codex-resume-probe\_logs`（`ro_resume.stdout.jsonl` 為關鍵煙槍）；Codex 反方 transcript `~/.claude/super-mode-logs/codex_consult_20260708_135355_ce8a75.txt`（高信心同意中止）。
...
- [x] T2：schema ×3＋fixture 雙向驗證＋文件接線（2026-07-07；含 commit 前審查修訂：flag 平台差異＋README 路徑前綴）。**T2b 已做（2026-07-09，使用者要求）**：consult 加 `-SchemaFile`(ps1)/`-s`(sh) → `--output-schema`；Windows 對 `$SchemaFile`＋既有 `$Dir` 加 cmd 注入守衛（拒 `% ! " & | < > ^`，fail-closed，因值進 `cmd /c` 會展開 `%VAR%`）；unix argv-safe 免守衛；新增 `tests/consult-schema.tests.{ps1,sh}`（win 5/5＋unix 2/2，codex-free）；`--sandbox read-only --ephemeral` 未動；Codex 反方審 BLOCK→硬化→PROCEED
- [x] T3：S1–S2 rg 判準 ×3（2026-07-07；fallback 經 commit 前審查改 fail-closed）
- [x] T4：S1–S2 rg 判準 ×3（2026-07-07）
- [x] T7：S1 基線記錄＋S2 六點落實＋S3 全綠（基線＋新增）＋S4 live 驗證（2026-07-08 新 session 開超級模式 scope=repo live 驗證**通過**：run-gate 55/55＋stdin driver 14/14＋真 MCP-through-harness 非白名單寫入 harddeny/唯讀 allow＋無憑證 Write deny 無 fail-open＋-Off 後清乾淨）
- [x] T5：S0 GATE **FAIL**（2026-07-08）→ **T5 結案、未實作 S1–S4**（`codex exec resume` 不繼承 read-only sandbox；見 §0.1）
- [x] T6：三判準命中（2026-07-07）
- [x] 同步矩陣逐格核對；§0.5 第 7、8 條全過（2026-07-08：三平台結構一致；`review-output.schema.json` 三平台 identical；`run-gate-tests.js` 統一為三平台 byte-identical（git diff --no-index 空）；`gate-cases.json` 差異＝平台專屬路徑案例 win55/mac63/linux65（已知）；.ps1 parse 0 錯＋BOM 過）
- [x] §6 本機同步完成（2026-07-08：本機 `~/.claude/skills/超級模式` 全量鏡像 repo/windows，hash byte-identical 驗證過、.ps1 BOM 保留、smoke test 過；hook 早於 T7-S4 同步。備份＝`~/.claude/skill-backups/超級模式.bak-20260708-174701`）
...
| **T5 resume（現行設計）** | **2026-07-08 GATE FAIL 實測**：`codex exec resume` 不繼承 read-only sandbox（resumed turn exit0 寫入成功）；session 連續≠sandbox 連續。不採「只靠 `.super-mode-active` guard 擋 resume」（單點防護放在 gate 無條件信任 codex-consult 的路徑上，過脆）、不採狀態檔 `sandbox` 欄位當安全證明 | 先新增並通過 per-resume sandbox override GATE（`-c sandbox_mode=read-only`＋fail-closed＋三平台一致）；未過維持 `--ephemeral` 單輪（見 §0.1） |
