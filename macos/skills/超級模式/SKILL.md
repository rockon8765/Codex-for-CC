...
- **派工前先確認 Codex 版本與能力面**：跑 `scripts/codex-check.sh`。**「有新版」是中性情報、不是更新指令**——更新屬選擇性系統變更、可能造成參數/外掛/行為漂移 → **先問使用者**。漂移警示屬提醒非閘門（姿態 A）。快取與 `-f`／`--update-baseline` 的判讀見 `references/orchestration.md` §3；各平台實作狀態見 [README「功能差距」](https://github.com/rockon8765/Codex-for-CC/blob/main/README.md#功能差距各平台實作狀態)。
