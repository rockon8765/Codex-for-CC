...
- **硬性強制**：超級模式啟用時，consult-gate hook（`~/.claude/hooks/super-mode-consult-gate.js`，經 `~/.claude/settings.json` 註冊）會攔下**所有會改變狀態的動作**（檔案寫入、非唯讀 shell、MCP 寫入／外發、排程／發佈／worktree 內建工具），要求 20 分鐘內的諮詢憑證；唯讀動作自動放行，**未知一律 default-deny**。`codex-exec.sh` 派工同樣要憑證（只有唯讀的 codex-consult / codex-check 與 super-mode 開關無條件放行）。收尾動作（commit / push / merge / publish / deploy）放行後憑證**降為只剩 3 分鐘**（同一條指令內 commit+push 不受影響），逼下一個里程碑重新諮詢。scratchpad 與 `~/.claude`（安全關鍵檔與會被自動執行的檔名除外）寫入豁免。**被擋時照 hook 的拒絕訊息做**——它會給出當下該跑的指令。完整攔截面枚舉、豁免細則與 pathless 取捨見 `references/orchestration.md` §3.5。
