...
t_h4_missing_latest_field() {   # 缺 latest=/verdict= 的 format=2 行 → miss（全欄位驗證）
  setup; printf 'format=2 installed=0.144.1 smoke=OK at 2026-07-13T00:00:00+0800\n' > "$fake_home/.claude/.codex-check-last"
  invoke_check noforce
...
t_h1_huge_banner_no_crash() {      # >64KB banner → 顯示管線 SIGPIPE 不得殺腳本
  setup; big="$(awk 'BEGIN{ s="wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww"; while (length(out) < 100000) out = out s; print out }')"
