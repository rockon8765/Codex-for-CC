...
>
> ⚠️ **不要拿你已安裝的那份 `matcher-contract` 當驗收依據。** 2026-07-28 以前安裝的版本
> 候選清單裡還有 `settings.local.json`，會假綠；而且它**不認識 `--live`，收到會靜默忽略後照樣 PASS**
> （2026-08-08 對 `1aeb010` 實測）。請用**你 checkout 的這份**跑 `--live`，並核對它印出來的兩條路徑。
>
> ⚠️ **重裝不是冪等的**：`settings.json` 已經有一筆 gate 時再跑一次安裝會變成兩筆。
> 判斷表見 [`docs/AI-INSTALL.md`](docs/AI-INSTALL.md) 步驟 2。
