...
> **macOS**：**部分驗證，不可讀成整批已驗**。中間 commit `9491719` 已在 macOS 26.6 arm64／內建 `bash 3.2.57`（`which -a bash` 只有 `/bin/bash`）原生跑過 `run-posix.sh` **67/67 exit 0**，並做過帶對照組的變異注入牙齒檢查。**但其後新增的 M5 前置檢查（→68 案）尚未在 macOS 跑過**，`run-posix.sh` 的 blob 已不是 Mac 驗過的那一份。要視為完整驗證，請在 Mac 上對**目前 tip** 複跑 `bash tests/ai-install/run-posix.sh`（應為 68/68）。
