...
---
**本機備註：** Codex CLI 裝於 `C:\npm`（三支腳本實際呼叫 `C:\npm\codex.cmd`，經 `cmd.exe /d /s /c` 把簡報以 stdin 重導向餵入，刻意避開 `codex.ps1` shim——2026-09-14 訂正，原文誤寫 `codex.ps1`）；prompt 一律走 STDIN + fresh child cmd、加 `--skip-git-repo-check`（腳本已封裝這些坑）。Codex 沙箱**進不到 WSL UNC 路徑**（`\\wsl.localhost\...`）→ 改由 Claude 讀檔、把證據餵給 Codex 做唯讀第二意見。諮詢/派工逐字稿與最終回覆都在 `~/.claude/super-mode-logs/`。
